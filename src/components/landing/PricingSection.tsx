...
// Benefícios do Teste Grátis (com limitações)
const trialBenefits = [
  {
    icon: CheckCircle2,
    text: "Acesso limitado a 1 dispositivo",
    highlight: false,
    limited: true,
  },
  {
    icon: CheckCircle2,
    text: "Limite de 5 empréstimos ou clientes",
    highlight: false,
    limited: true,
  },
  {
  ...
  {
    icon: CheckCircle2,
    text: "Suporte no WhatsApp (horário comercial)",
    highlight: false,
    limited: true,
  },
];

// Benefícios dos Planos Pagos (sem limitações)
const paidBenefits = [
  {
    icon: CheckCircle2,
...
export default function PricingSection({ MONTHLY_PRICE, KIVANO_PURCHASE_URL }: PricingSectionProps) {
  ...
  return (
    <section id="pricing" className="py-16 sm:py-20 lg:py-32 xl:py-40 relative overflow-hidden" style={{ background: 'transparent' }}>
      {/* Animated Background */}
      <div className="absolute inset-0">
        <div className="absolute inset-0" style={{ background: 'radial-gradient(circle at center, rgba(44, 89, 130, 0.1), transparent 60%)' }} />
        <motion.div
          ...
          {pricingPlans.map((plan, index) => {
            ...
            return (
              ...
              >
                ...
                {/* Main Card */}
                <div className="relative rounded-2xl p-6 sm:p-8 overflow-hidden h-full flex flex-col" style={{
                  background: plan.popular 
                    ? 'linear-gradient(135deg, rgba(59, 130, 246, 0.15), rgba(20, 184, 166, 0.15))'
                    : 'rgba(26, 58, 82, 0.3)',
                  backdropFilter: 'blur(20px)',
                  border: plan.popular 
                    ? '2px solid rgba(59, 130, 246, 0.4)' 
                    : '1px solid rgba(76, 142, 196, 0.2)',
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    ...
                    <div className="space-y-2 mb-6 flex-grow">
                      {(plan.id === 'trial' ? trialBenefits : paidBenefits).slice(0, 5).map((benefit, benefitIndex) => (
                        <div
                          key={benefitIndex}
                          className="flex items-start gap-2"
                        >
                          <CheckCircle2 className="w-4 h-4 flex-shrink-0 mt-0.5" style={{
                            color: benefit.limited ? '#fbbf24' : '#10f981',
                            filter: benefit.limited 
                              ? 'drop-shadow(0 0 4px rgba(251, 191, 36, 0.6))' 
                              : 'drop-shadow(0 0 4px rgba(16, 249, 129, 0.6))'
                          }} />
                          <span className={`text-xs sm:text-sm ${benefit.limited ? 'text-yellow-300' : 'text-gray-300'}`}>
                            {benefit.text}
                          </span>
                        </div>
                      ))}
                      <p className="text-xs text-gray-400 italic mt-3">
                        {plan.id === 'trial' ? '+ Recursos básicos inclusos' : '+ Todos os recursos inclusos'}
                      </p>
                    </div>
                    ...
                    >
                      <button className={`relative w-full ${
                        ...
                        <span className="relative z-10">
                          {plan.id === 'trial' ? 'Começar Teste Grátis' : `Assinar ${plan.name}`}
                        </span>
                    ...
                    <div className="mt-4 pt-4 border-t border-white/10 space-y-1.5">
                      <div className="flex items-center gap-2 text-gray-400">
                        <Zap className="w-3 h-3 flex-shrink-0" style={{
                          color: '#4a90ff',
                          filter: 'drop-shadow(0 0 3px rgba(74, 144, 255, 0.6))'
                        }} />
                        <span className="text-xs">Acesso imediato</span>
                      </div>
                      <div className="flex items-center gap-2 text-gray-400">
                        <CheckCircle2 className="w-3 h-3 flex-shrink-0" style={{
                          color: '#4a90ff',
                          filter: 'drop-shadow(0 0 3px rgba(74, 144, 255, 0.6))'
                        }} />
